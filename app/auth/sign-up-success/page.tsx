...
export default function SignUpSuccess() {
  ...
  async function resend() {
    if (!email) {
      setStatus("No encontramos el correo de registro. Regístrate nuevamente.")
      return
    }
    setSending(true)
    setStatus(null)
    ...
    <main className="min-h-screen bg-background flex items-center justify-center px-6 py-16">
      <div className="max-w-md text-center">
        ...
        <p className="text-muted-foreground leading-relaxed mb-8">
          Te enviamos un enlace de verificación a <span className="text-foreground">{email || "tu correo"}</span>. Si no
          llega en 2 minutos, solicita reenvío.
        </p>
